        ...
        if($stmt = mysqli_prepare($conn2, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){
                    ...
                    mysqli_stmt_bind_result($stmt, $id, $email, $hashed_hiddenkey); //hash here and verify later that the password matches the hashed one
                    if(mysqli_stmt_fetch($stmt)){
                        if(hiddenkey_verify($hiddenkey, $hashed_hiddenkey)){ //hash password here so that it is encrypted
